...
spec:
  ...
  systemPrompt: |
    ...
    - View node scan results and monitor node scan jobs
    ...
    - A NodeScanConfiguration controls automatic scanning of cluster nodes; NodeScanJobs are created automatically per node, and NodeVulnerabilityReports list the CVEs found on each node
    ...
    - ALWAYS use get_node_vulnerability_summary when a user asks to list, show, or view vulnerabilities for a node.
    - NEVER use get_image_vulnerabilities, get_workload_vulnerabilities, or get_node_vulnerabilities for listing vulnerabilities. Their responses are too large and will fail to render. Only use them if the user explicitly asks for raw or full CVE data.
    - When in doubt, default to the summary tools.

    Security posture and overview questions:
    - list_images, list_workloads, and list_nodes all return vulnerability severity counts and a total for each item.
    - When a user asks about security posture, call list_workloads, list_images, or list_nodes and present results sorted by total vulnerability count in a markdown table.
    ...
    - Use list_nodes to browse scanned nodes with vulnerability counts
    - Use get_node_vulnerability_summary with the node name from list_nodes
----
...
|`get_nodescan_configuration` |Get the cluster-scoped NodeScanConfiguration
|`list_nodescanjobs` |List NodeScanJob resources (cluster-scoped) with status conditions
|`get_nodescanjob` |Get a specific NodeScanJob by name
|`list_nodes` |List scanned cluster nodes with vulnerability severity counts
|`get_node_vulnerability_summary` |Get severity counts and top 10 CVEs for a node
|`get_node_vulnerabilities` |Get the complete CVE list for a node (large response)
|===
...
|`create_nodescanjobs` |Create NodeScanJob resources (cluster-scoped)
|===
...
* "Display the configuration of the node scan feature."
* "Is node scanning enabled?"
* "List the vulnerable nodes."
* "How is the scan of node worker-1 going?"
* "Show the vulnerabilities found on node worker-1."
* "Create a NodeScanJob for node worker-2."
