...
[NOTE]
====
When `spec.repositories` is set, the obsolete-image cleanup is skipped for the entire `ScanJob`. Images that are no longer present in the registry are not deleted from the cluster while a targeted scan runs; they are reconciled the next time a non-targeted `ScanJob` (scheduled or on-demand) runs against the same `Registry`.
====
...
To make {sboms-name} work with these registries, you can manually specify the repositories you want to scan, instead of pulling the catalog.
...
To lighten the workload of {sboms-name}, you can specify which platforms you want to scan, instead of discovering them and consequently scan.
...
It is widely used in https://kubernetes.io/docs/reference/using-api/cel/[Kubernetes], making CEL a convenient alternative to out-of-process mechanisms, such as webhooks, for many extensibility use cases.
